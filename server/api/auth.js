...
export async function userFromToken(t) {
  if (!t) return null;
  const s = await one('SELECT * FROM sessions WHERE token = ?', [t]);
  if (!s) return null;
...
export async function createUser({ email, name, password, role = 'staff' }) {
  ...
  if (await one('SELECT id FROM users WHERE email = ?', [mail])) throw new HttpError(400, 'Ese correo ya está registrado');
  const user = {
