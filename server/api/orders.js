...
export function listOrders(query = {}) {
  ...
  if (query.q) {
    where.push('(o.code LIKE ? OR o.customer_name LIKE ? OR o.phone LIKE ? OR o.city LIKE ?)');
    const like = `%${query.q}%`;
    params.push(like, like, like, like);
  }
  const limit = Math.min(500, toInt(query.limit, 200) || 200);
  const sql = `${ENRICH} ${where.length ? 'WHERE ' + where.join(' AND ') : ''} ORDER BY o.created_at DESC LIMIT ${limit}`;
...
export function countsByStatus() {
  const rows = all('SELECT status, COUNT(*) n, SUM(total) revenue FROM orders GROUP BY status');
  const out = { all: 0 };
...
export function getOrder(oid) {
  const o = one(`${ENRICH} WHERE o.id = ? OR o.code = ?`, [oid, oid]);
  if (!o) throw notFound('Pedido no encontrado');
  return { ...o, events: all('SELECT * FROM order_events WHERE order_id = ? ORDER BY created_at DESC', [o.id]) };
}

function logEvent(orderId, type, message, actor = 'sistema') {
 ...
 * `source` distingue el origen para el registro de eventos.
 */
export function createOrder(body, { source = 'panel', actor = 'sistema' } = {}) {
  ...
  const page = body.page_id ? one('SELECT * FROM pages WHERE id = ? OR slug = ?', [body.page_id, body.page_id]) : null;
  const productId = body.product_id || page?.product_id || null;
  const product = productId ? one('SELECT * FROM products WHERE id = ?', [productId]) : null;
  ...
  const customer = upsertCustomer({ name, phone, email: body.email, department: body.department, city: body.city, address: body.address, total });
  ...
  insert('orders', order);
  logEvent(order.id, 'created', source === 'landing' ? 'Pedido recibido desde la landing' : 'Pedido creado manualmente', actor);

  if (product && product.stock > 0) {
    run('UPDATE products SET stock = MAX(0, stock - ?) WHERE id = ?', [qty, product.id]);
  }
  return getOrder(order.id);
}

export function updateOrder(oid, body, actor = 'sistema') {
  const existing = one('SELECT * FROM orders WHERE id = ?', [oid]);
  ...
  update('orders', oid, patch, [...EDITABLE, 'updated_at']);

  if (patch.status && patch.status !== existing.status) {
    logEvent(oid, 'status', `${ORDER_STATUS[existing.status].label} → ${ORDER_STATUS[patch.status].label}`, actor);
    if (patch.status === 'delivered') recomputeCustomer(existing.customer_id);
  }
  if (patch.notes != null && patch.notes !== existing.notes) logEvent(oid, 'note', 'Nota actualizada', actor);
  return getOrder(oid);
}

export function bulkStatus(ids, status, actor = 'sistema') {
  if (!ORDER_STATUS[status]) throw new HttpError(400, 'Estado inválido');
  let n = 0;
  for (const oid of (Array.isArray(ids) ? ids : []).slice(0, 500)) {
    try { updateOrder(oid, { status }, actor); n++; } catch { /* ignora los que ya no existen */ }
  }
  return { updated: n };
}

export function deleteOrder(oid) {
  const n = run('DELETE FROM orders WHERE id = ?', [oid]).changes;
  if (!n) throw notFound('Pedido no encontrado');
  return { ok: true };
}

export function addNote(oid, message, actor) {
  if (!one('SELECT id FROM orders WHERE id = ?', [oid])) throw notFound('Pedido no encontrado');
  logEvent(oid, 'note', clean(message, 600), actor);
  return getOrder(oid);
}

/* ── Clientes ─────────────────────────────────────────────────────────── */

function upsertCustomer({ name, phone, email, department, city, address, total }) {
  const existing = one('SELECT * FROM customers WHERE phone = ?', [phone]);
  const now = nowISO();
  if (existing) {
    run(`UPDATE customers SET name = ?, email = COALESCE(NULLIF(?,''), email),
...
export function recomputeCustomer(customerId) {
  if (!customerId) return;
  const r = one(`SELECT COUNT(*) n, COALESCE(SUM(total),0) spent FROM orders
                 WHERE customer_id = ? AND status = 'delivered'`, [customerId]);
  run('UPDATE customers SET total_spent = ? WHERE id = ?', [r.spent, customerId]);
}
...
export function ordersCSV(query = {}) {
  const { orders } = listOrders({ ...query, limit: 5000 });
